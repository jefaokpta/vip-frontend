...
import {decodeEmailBase64, maskEmail} from '@/util/utils';
...
@Component({
    ...
    template: ` <svg
        ...
        <div class="px-8 min-h-screen flex justify-center items-center">
            ...
            >
                <div class="mb-6">
                    <div class="text-surface-900 dark:text-surface-0 text-xl font-bold mb-2">Confirme seu email</div>
                    <span class="text-surface-600 dark:text-surface-200 font-medium">
                        Enviamos um código para <b>{{ maskedEmail }}</b>
                    </span>
                </div>
                <form [formGroup]="form" (ngSubmit)="submit()">
                    <div class="flex flex-col items-center gap-4">
                        <p-input-otp formControlName="code" [integerOnly]="true" [length]="6"></p-input-otp>

                        <div class="flex gap-4">
                            <p-button type="submit" label="Confirmar" [disabled]="form.invalid || pending">
...
export class ConfirmEmailPage implements OnInit {
    ...
    ngOnInit(): void {
        ...
        const encodedEmail = this.activatedRoute.snapshot.queryParamMap.get('e') ?? '';
        this.email = decodeEmailBase64(encodedEmail);
