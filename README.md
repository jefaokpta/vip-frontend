...
- WebPhone (JsSIP) em `src/app/webphone/webphone.service.ts`:
  ...
  - Integração com `UserService` para identificar o usuário e `PeerDashboardService` para obter token da chamada;
...
- Serviços utilitários em `src/app/services` (UserService, PeerDashboardService, etc.).
